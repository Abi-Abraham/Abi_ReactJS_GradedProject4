...
const MovieListItem = ({ movies, path, onRemove }: Props) => {
    ...
    const addMovieToFavourite = async (event: React.MouseEvent<HTMLButtonElement>) => {
        ...
        try {
            ...
            if (movieByTitle !== null) {
                toast.error("Already added to favourites...", { autoClose: toastTimeout, position:"bottom-right" })
                return;
            }
            ...
            toast.success("Successfully added to favourites...", { autoClose: toastTimeout,position:"bottom-right",transition:Slide })
        }
        catch (errormsg: any) {
            toast.error("Not added the favourites...", { autoClose: toastTimeout,
                position:"bottom-right",transition:Slide })
        }
    };
    ...
    const removeMovieFromFavourite = async (event: React.MouseEvent<HTMLButtonElement>) => {
        ...
        try {
            ...
            toast.success("Successfully removed from favourites...", { autoClose: toastTimeout,position:"bottom-right",transition:Slide })
            onRemove(movies.title);
        }
        catch (errormsg: any) {
            toast.error("Unable to remove from favourites...", { autoClose: toastTimeout,position:"bottom-right",transition:Slide })
        }
    };
